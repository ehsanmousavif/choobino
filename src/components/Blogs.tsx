  ...
  const BlogCard = ({ blog }: { blog: BlogType }) => {
    return (
      <Link
        href={blog.link}
        className="embla__slide grow-0 shrink-0 basis-[70%] flex flex-col gap-8 mx-2 md:basis-[calc(100%/2)] lg:basis-[calc(100%/4-16px)]"
        draggable="false"
      >
        <Image
          alt={blog.title}
    ...
    return (
      <section className="rounded-3xl w-full flex flex-col relative gap-12 mt-60 mb-64">
        <div className="w-full flex justify-between h-18 items-center ">
          <b className="text-2xl max-md:m-auto md:text-4xl">مقالات اخیر</b>
          <Button
            variant="bordered"
