...
import { useBlogProgressButton } from "./useBlogProgressButton";
...
export default function ProductCarousel() {
  ...
  return (
    <section className="rounded-3xl w-full flex flex-col relative gap-12 my-24">
      <div className="w-full flex items-center justify-between">
        <div className="flex flex-col items-center justify-center md:flex-row md:items-center md:justify-start h-18 w-full">
          <h2 className="text-2xl max-md:m-auto md:text-4xl">محصولات ویژه</h2>
          <div className="flex flex-row items-end gap-4 max-w-2xl">
            ...
            <Button
              ...
              onPress={() => setActiveButton("special")}
            >
              {" "}
              فروش ویژه{" "}
            </Button>{" "}
      ...
      <div className="overflow-hidden" ref={emblaRef}>
        <div className="flex gap-6 px-2 md:px-0">
          {productCardData.map((item, index) => (
            <div
              key={index}
              className="flex-[0_0_70%] sm:flex-[0_0_40%] md:flex-[0_0_25%]"
            >
              <ProductCard {...item} />
            </div>
          ))}
